...
class TestMain:
    ...
    def test_download(self):
        ...
        with temppath() as tpath:
            main(["download", "foo", tpath, "--silent", "--threads", "1"], self.client)
            self._dircmp(tpath)
    ...
    def test_download_stream_multiple_files(self):
        with pytest.raises(SystemExit):
            self.client.upload("foo", self.dpath)
            main(["download", "foo", "-", "--silent", "--threads", "1"], self.client)
    ...
    def test_upload(self):
        main(["upload", self.dpath, "bar", "--silent", "--threads", "1"], self.client)
        with temppath() as tpath:
